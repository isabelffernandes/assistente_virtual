...
class API {
	...
	public static function get_items_count(){
		$trigger_time = DB::get_transient( 'templately_trigger' );
		if( $trigger_time !== false ) {
			...
			return DB::get_option( '_templately_item_counts', $defaults );
		}
		...
		if( isset( $data['data']['getCounts'] ) ) {
			DB::set_transient( 'templately_trigger', true );
			$new_array = [
				'items' => [],
				'blocks' => [],
				'pages' => [],
			...
			DB::update_option('_templately_item_counts', $new_array );
			return $new_array;
		}
	}
	/**
